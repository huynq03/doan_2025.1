# dieu_khien_with_object.py
# -*- coding: utf-8 -*-
"""
PD + Feed-forward controller cho quadrotor + gripper (mặt phẳng x–z),
CÓ THÊM LOGIC GẮP VẬT 27g.

Sau khi gripper đóng (beta < -20°), khối lượng vật 27g sẽ được cộng thêm vào
hệ thống để mô phỏng việc gắp vật.
...
class PDFFController:
    def __init__(
        ...
        # ==== THÊM LOGIC GẮP VẬT ====
        self.object_mass = 0.027  # kg (27 gram)
        self.gripped = False
        self.grip_time = 2.0  # Gắp vật tại thời điểm t=2s
        print(f"[INFO] Khối lượng vật: {self.object_mass*1000:.1f}g")
        print(f"[INFO] Thời điểm gắp: t = {self.grip_time}s")
    ...
    def mophong(self, save_csv: Optional[str] = None, animate: bool = False):
        """
        Harness tối giản chạy mô phỏng phẳng với mo_phong_with_object (nếu có).
        ...
        THÊM: Sau khi gắp vật (beta < -20°), sẽ cộng thêm 27g vào khối lượng.
        """
        try:
            from mo_phong_with_object import jax_dynamics_matrix
            print("[INFO] Đã import mo_phong_with_object.py")
        except Exception as e:
            # Fallback: dùng động lực học tối giản bằng NumPy nếu không import được mo_phong (ví dụ thiếu JAX)
            print(f"[WARN] Không import được mo_phong_with_object.py ({e}). Dùng mô phỏng fallback đơn giản.")

            def jax_dynamics_matrix(state: np.ndarray, control: np.ndarray, dt: float, m_g_override: float = None) -> np.ndarray:
                """
                ...
                m_g_override: khối lượng gripper + vật (nếu có)
                """
                m_q = float(self.params.get("m_q", 0.5))
                m_g_base = float(self.params.get("m_g", 0.158))
                m_g = m_g_override if m_g_override is not None else m_g_base
        ...
        for i in range(len(self.t)-1):
            ...
            if not self.gripped and current_time >= self.grip_time:
                self.gripped = True
                beta_deg = np.rad2deg(state[6])
                print(f"[GRIP] Đã gắp vật tại t={current_time:.3f}s, beta={beta_deg:.1f}°. Cộng thêm {self.object_mass*1000:.1f}g vào khối lượng.")
            
            # Tính khối lượng gripper hiệu dụng (bao gồm vật nếu đã gắp)
            m_g_base = float(self.params.get("m_g", 0.158))
            m_g_effective = m_g_base + (self.object_mass if self.gripped else 0.0)
            ...
            # ==== TÍCH PHÂN MỘT BƯỚC VỚI KHỐI LƯỢNG ĐỘNG ====
            control = np.array([u1, u2, tau], dtype=float)
            state = np.array(jax_dynamics_matrix(state, control, dt=self.dt, m_g_override=m_g_effective), dtype=float)
        ...
        if save_csv:
            if not save_csv.lower().endswith('.csv'):
                ...
                save_csv = os.path.join(folder_path, "minsnap_results\\ketqua_with_object.csv")
            else:
        ...
        if animate:
            try:
                from mo_phong_with_object import animate as mo_phong_animate
                mo_phong_animate(states, cmds[:, :3], target=(self.x_qd[-1], self.z_qd[-1]), dt=self.dt, save_frames=True, output_dir="../media/animation_frames_with_object")
            except Exception as e:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PD + FF quad controller (WITH 27g OBJECT)")
    ...
    parser.add_argument("--simulate", action="store_true", help="Chạy mô phỏng với mo_phong_with_object.py")
