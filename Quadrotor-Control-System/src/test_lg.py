...
def run_simulation(lg_value):
    ...
    QUY TRÌNH:
    1. Đọc quỹ đạo tham chiếu từ flat_outputs.csv
    2. Chạy PD + Feedforward Controller
    3. Thêm nhiễu Gaussian vào đo lường (sigma_x, sigma_z, sigma_theta, sigma_beta)
    4. Tích phân dynamics với JAX
    5. Lưu quỹ đạo THỰC TẾ (đã qua nhiễu + controller) vào CSV
    """
    ...
    print(f"{'='*50}")
    print(f"  → Quỹ đạo sẽ chạy qua:")
    print(f"     1. PD + Feedforward Controller")
    print(f"     2. Nhiễu Gaussian (sigma=0.15)")
    print(f"     3. Dynamics tích phân (JAX)")
    print(f"  → Kết quả: Quỹ đạo THỰC TẾ (có nhiễu + sai số)")
...
def plot_comparison(trajectories, lg_values):
    ...
    ax1.set_title('Quỹ đạo chuyển động', fontsize=14)
    ...
    # Lưu hình
    output_file = 'minsnap_results/sosanh_lg.png'
...
def main():
    ...
    print(f"\nBiểu đồ so sánh: minsnap_results/sosanh_lg.png")
