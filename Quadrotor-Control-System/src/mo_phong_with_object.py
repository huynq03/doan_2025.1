...
# ==== Dynamics (CÓ HỖ TRỢ KHỐI LƯỢNG ĐỘNG) ====
def jax_dynamics_matrix(state, control, dt=dt, m_g_override=None):
    """
    Dynamics với hỗ trợ khối lượng động (m_g_override).
    Khi m_g_override != None, sử dụng giá trị này thay vì m_g mặc định.
    Điều này cho phép mô phỏng việc gắp vật có khối lượng khác nhau.
    """
    y, y_dot, z, z_dot, phi, phi_dot, beta, beta_dot = state
    u1, u2, tau = control
    
    # Sử dụng m_g_override nếu được cung cấp, nếu không dùng m_g mặc định
    m_g_use = m_g_override if m_g_override is not None else m_g
    M = m_q + m_g_use
    ...
    D = jnp.array([
        ...
        [0, M, 0, -L_g*m_g_use*c],
        [0, 0, J_q, 0],
        [-L_g*m_g_use*s, -L_g*m_g_use*c, 0, J_g + L_g**2 * m_g_use]
    ], dtype=state.dtype)
    C = jnp.array([
        [0, 0, 0, -L_g*m_g_use*c * beta_dot],
        [0, 0, 0,  L_g*m_g_use*s * beta_dot],
        [0, 0, 0, 0],
        [0, 0, 0, 0]
    ], dtype=state.dtype)
    G = jnp.array([0, g*M, 0, -g*L_g*m_g_use*c], dtype=state.dtype)
...
# ==== Visualization ====
def animate(states, controls, target=(5.0, 5.0), dt=dt, save_frames=False, output_dir="../media/animation_frames", playback_speed=4.0):
    ...
    # Dùng blit=True để tăng tốc animation (chỉ vẽ lại phần thay đổi)
